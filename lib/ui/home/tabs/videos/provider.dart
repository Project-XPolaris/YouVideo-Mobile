...
class HomeVideosProvider extends ChangeNotifier {
  ...
  String GridViewType =
      ApplicationConfig().config.HomeVideosGridViewType ?? "Medium";
  String layoutType = ApplicationConfig().config.HomeVideosLayout ?? "Default";
  ...
  updateLayoutType(String newType) {
    layoutType = newType;
    ConfigData newConfig = ApplicationConfig().config;
    newConfig.HomeVideosLayout = newType;
    ApplicationConfig().UpdateData(newConfig);
    notifyListeners();
  }
}
